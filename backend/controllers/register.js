...
export const getItems = async (req, res) => {
  try {
    ...
    const items = await Register.find({ studentMail: studentMail });
...
export const getAllItems = async (req, res) => {
  ...
  try {
    ...
    const items = await Register.find({ classCode: classCode });
    res.status(200).json({ items });
  } catch (error) {
    console.log(error);
  }
...
export const addItem = asyncWrapper(async (req, res) => {
  ...
  const item = await Register.create({ name, file, studentMail, classCode });
